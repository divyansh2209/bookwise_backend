...
class Book():
    ...
    def returnChapters(self):
        chapterNums = self.zeroPad(range(1, len(self.chapters)+1))
        logging.debug('Returning chapter headings: %s' % chapterNums)
        ...
        if self.nochapters:
            ...
            for chapter in self.chapters:
                ...
                text += chapter + '\n'
            print(json.dumps({"extracted_text": text}, ensure_ascii=False, indent=2))
        else:
            chapters_dict = {}
            for i, chapter in enumerate(self.chapters, start=1):
                chapter_content = '\n'.join(chapter)
                chapters_dict[str(i)] = chapter_content
            
            print(json.dumps(chapters_dict, ensure_ascii=False, indent=2))
